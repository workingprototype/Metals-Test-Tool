...
try {
    ...
    // In a real production environment, you'd log this error instead of showing it
 ...
 * @param array $queryParams The existing GET parameters to preserve in links.
 * @return string The generated HTML for the pagination.
 */
...
// Search & filter parameters from GET request
$search = $_GET['search'] ?? '';
...
// Default date range to today if no filters are applied
...
// This section runs BEFORE any HTML is output.
// =================================================================

// Handle Delete Record (SECURE)
if (isset($_GET['delete_id'])) {
    $delete_id = $_GET['delete_id'];
    $stmt = $conn->prepare("DELETE FROM test_reports WHERE sr_no = ?");
    $stmt->bind_param("s", $delete_id);
    if ($stmt->execute()) {
        // Use a session flash message for better UX, but script alert is simpler here
        echo "<script>alert('Record deleted successfully'); window.location.href='reports.php';</script>";
    } else {
        echo "<script>alert('Error deleting record: " . $stmt->error . "'); window.location.href='reports.php';</script>";
    }
    $stmt->close();
...
if (isset($_GET['export'])) {
    // Build the query for export first (without LIMIT)
    $where_clauses = [];
    $params = [];
    $types = '';
    ...
    $export_sql = "SELECT * FROM test_reports";
    ...
    if ($_GET['export'] == 'excel') {
        ...
        $sheet->setTitle('Test Reports');

        $headers = ['Sr No', 'Report Date', 'Name', 'Sample', 'Mobile', 'Alt Mobile', 'Metal Type', 'Weight', 'Purity / Gold %', 'Silver', 'Zinc', 'Copper', 'Others', 'Platinum', 'Rhodium', 'Iridium', 'Ruthenium', 'Palladium', 'Lead', 'Tin', 'Cadmium', 'Nickel', 'Total Karat'];
        ...
        while ($row = $export_result->fetch_assoc()) {
            ...
            $sheet->setCellValue('W'.$rowCount, $row['total_karat']);
            $rowCount++;
        }

        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="test_reports.xlsx"');
    ...
    // EXPORT TO PDF
    if ($_GET['export'] == 'pdf') {
        // FPDF is very basic; for complex tables, a library like TCPDF or mPDF is better.
        // This is a simplified version.
        $pdf = new FPDF('L', 'mm', 'A4'); // Landscape mode
        $pdf->AddPage();
        $pdf->SetFont('Arial', 'B', 8);

        $headers = ['SrNo', 'Date', 'Name', 'Sample', 'Mobile', 'Metal', 'Weight', 'Gold%', 'Silver', 'Copper', 'Zinc', 'Others']; // Simplified headers for PDF
        foreach ($headers as $header) {
            $pdf->Cell(23, 7, $header, 1);
        }
        $pdf->Ln();

        $pdf->SetFont('Arial', '', 8);
        while ($row = $export_result->fetch_assoc()) {
            $pdf->Cell(23, 6, $row['sr_no'] ?? 'N/A', 1);
            $pdf->Cell(23, 6, date('d-m-Y', strtotime($row['report_date'] ?? '')), 1);
            $pdf->Cell(23, 6, $row['name'] ?? 'N/A', 1);
            $pdf->Cell(23, 6, $row['sample'] ?? 'N/A', 1);
            $pdf->Cell(23, 6, $row['mobile'] ?? 'N/A', 1);
            $pdf->Cell(23, 6, $row['metal_type'] ?? 'N/A', 1);
            $pdf->Cell(23, 6, $row['weight'] ?? 'N/A', 1);
            $pdf->Cell(23, 6, $row['gold_percent'] ?? 'N/A', 1);
            $pdf->Cell(23, 6, $row['silver'] ?? 'N/A', 1);
            $pdf->Cell(23, 6, $row['copper'] ?? 'N/A', 1);
            $pdf->Cell(23, 6, $row['zinc'] ?? 'N/A', 1);
            $pdf->Cell(23, 6, $row['others'] ?? 'N/A', 1);
            $pdf->Ln();
        }
...
$count_sql = "SELECT COUNT(*) AS total FROM test_reports" . $sql_where;
...
$data_sql = "SELECT * FROM test_reports" . $sql_where . " ORDER BY report_date DESC, id DESC LIMIT ?, ?";
$stmt = $conn->prepare($data_sql);
if (!empty($params)) {
    $types .= 'ii'; // Add types for limit and start
...
<body>
    ...
    <div class="container mt-4">
        <div class="card">
            <div class="card-header">
                <h4 class="mb-0">Search & Filter Reports</h4>
            </div>
            <div class="card-body">
                <form method="GET" action="" class="row g-3 align-items-end">
                    ...
                    <div class="col-lg-3 col-md-12 d-flex flex-wrap gap-2">
                        ...
                        <a href="reports.php" class="btn btn-secondary flex-grow-1">Reset</a>
                    </div>
                </form>
            </div>
            <div class="card-footer d-flex justify-content-between align-items-center">
                <div>
                    <span class="badge bg-info">Total Records: <?php echo $total_rows; ?></span>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0): ?>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                <td>
                                    <a href="?delete_id=<?php echo htmlspecialchars($row['sr_no'] ?? ''); ?>" class="btn btn-danger btn-sm action-btn" onclick="return confirm('Are you sure you want to delete this record? This action cannot be undone.');">Delete</a>
                                </td>
                                ...
                                <td><?php echo htmlspecialchars($row['gold_percent'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['silver'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['zinc'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['copper'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['others'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['platinum'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['rhodium'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['iridium'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['ruthenium'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['palladium'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['lead'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['tin'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['cadmium'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['nickel'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($row['total_karat'] ?? 'N/A'); ?></td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="24" class="text-center">No records found matching your criteria.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    <!-- Bootstrap JS (using the newer bundle that includes Popper.js) -->
